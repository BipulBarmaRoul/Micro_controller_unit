...
/**
  ...
  * @brief          : combine all the headers and write the main case study.
  ******************************************************************************
  * @attention
  *
...
int main(void)
{
  ...
  //register code for KEYPAD INTERFACING
  ...
  while (1)
  {
	    //register code for RFID with SPI INTERFACING
	    main:
	    setCursor(0,0);
		delay(100);
		lcd_send_string("$ SWAPNA NIVAS $");
		setCursor(0,1);
		delay(100);
		lcd_send_string("ENTER CARD / KEY");
		if(rc522_checkCard(rfid_id))
		{
			...
			if((strcmp(data,rfd)))
			{
				...
				//HAL library code for DC SERVO MOTOR
				int e = 25;
				while(e<75)
				{
					htim2.Instance->CCR1 = e;
					e++;
					delay(400);
				}
				delay(60000);
			...
			else
			{
				...
				setCursor(0,1);
				lcd_send_string("Again Scan Card");
				delay(5000);
				lcd_clear();
			}
		}
		...
		else
		{
			...
			*portcODR &= ~(1<<0);
			...
			if(!(*portcIDR&(1<<7)))
			{
				...
				setCursor(0,0);
				delay(100);
				lcd_send_string("KEY IS CORRECT!!");
				setCursor(0,1);
				delay(100);
				lcd_send_string("Welcome To Home");
				delay(10000);
		...
		//HAL library code for PIR SENSOR INTERFACING
		uint8_t cnt = 0;
			...
			while(cnt<3)
			{
				if(cnt==1)
				{
					if((HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_8)==1) || (HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_9)==1))
					{
						...
						setCursor(0,1);
						delay(100);
						lcd_send_string("SomeOne Is Here!");
						HAL_GPIO_WritePin(GPIOC, GPIO_PIN_10,1);
						delay(50000);
						goto main;
					}
		...
		//HAL library code for RAIN SENSOR INTERFACING
		HAL_ADC_Start(&hadc1);
